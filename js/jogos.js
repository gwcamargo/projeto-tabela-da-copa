...
const preencherCardJogos = (lista, jogo) => {
    lista.querySelector('.grupo').innerHTML = `Grupo ${jogo.grupo}`
...
const renderizar = () => {
    listarJogos(url)
    .then(dado => {
        document.querySelector('.rodada').innerHTML = `${dado[0].rodada}ª`
        dado.map((jogo, indice) => {
            let lista = criarCardJogo()
            preencherCardJogos(lista, jogo)
        })
    })
}

// renderizando a nossa tela
renderizar()